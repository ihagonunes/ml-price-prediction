...
ADVANCED_FOLD_METRICS_FILE = REPORTS_DIR / "comfort_advanced_tscv_fold_metrics.csv"
ADVANCED_SUMMARY_FILE = REPORTS_DIR / "comfort_advanced_tscv_summary.csv"
ADVANCED_COMPARISON_FILE = REPORTS_DIR / "comfort_advanced_vs_baseline.csv"
ADVANCED_REPORT_FILE = REPORTS_DIR / "comfort_advanced_tscv_report.md"
COMFORT_CATEGORY_ID = 9
...
def load_comfort_baseline_summary() -> tuple[pd.DataFrame, pd.Series]:
    ...
    comfort_df = summary_df.loc[summary_df["CategoryID"] == COMFORT_CATEGORY_ID].copy()
    if comfort_df.empty:
        raise ValueError("Nao ha linhas de baseline para Uber Comfort no resumo atual.")

    best_row = comfort_df.sort_values(
        by=["MeanMAE", "MeanRMSE"],
        kind="stable",
    ).iloc[0]
    return comfort_df.reset_index(drop=True), best_row
...
def run_advanced_tscv() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    dataset_spec = CATEGORY_DATASETS[COMFORT_CATEGORY_ID]
    category_frame = load_category_feature_frame(COMFORT_CATEGORY_ID, config)
    ...
    for split_window in cv_windows:
        ...
        for model_name, model_object in model_registry.items():
            ...
            fold_rows.append(
                {
                    "CategoryID": COMFORT_CATEGORY_ID,
    ...
    _, best_baseline_row = load_comfort_baseline_summary()
...
def write_report(
    ...
    report_lines = [
        ...
        f"- Dataset: `{CATEGORY_DATASETS[COMFORT_CATEGORY_ID]['file_name']}`",
...
def main() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    log.info("=" * 60)
    log.info("Treinando modelos avancados para Uber Comfort")
    log.info("=" * 60)
    ...
    log.info("=" * 60)
    log.info("Modelos avancados de Uber Comfort concluidos")
    log.info("=" * 60)
    return fold_metrics_df, summary_df, comparison_df
