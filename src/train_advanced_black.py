...
ADVANCED_FOLD_METRICS_FILE = REPORTS_DIR / "black_advanced_tscv_fold_metrics.csv"
ADVANCED_SUMMARY_FILE = REPORTS_DIR / "black_advanced_tscv_summary.csv"
ADVANCED_COMPARISON_FILE = REPORTS_DIR / "black_advanced_vs_baseline.csv"
ADVANCED_REPORT_FILE = REPORTS_DIR / "black_advanced_tscv_report.md"
BLACK_CATEGORY_ID = 4
...
def load_black_baseline_summary() -> tuple[pd.DataFrame, pd.Series]:
    ...
    black_df = summary_df.loc[summary_df["CategoryID"] == BLACK_CATEGORY_ID].copy()
    if black_df.empty:
        raise ValueError("Nao ha linhas de baseline para Uber Black no resumo atual.")

    best_row = black_df.sort_values(
        by=["MeanMAE", "MeanRMSE"],
        kind="stable",
    ).iloc[0]
    return black_df.reset_index(drop=True), best_row
...
def run_advanced_tscv() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    dataset_spec = CATEGORY_DATASETS[BLACK_CATEGORY_ID]
    category_frame = load_category_feature_frame(BLACK_CATEGORY_ID, config)
    ...
    _, best_baseline_row = load_black_baseline_summary()
...
def write_report(
    ...
    report_lines = [
        ...
        f"- Dataset: `{CATEGORY_DATASETS[BLACK_CATEGORY_ID]['file_name']}`",
...
def main() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    log.info("=" * 60)
    log.info("Treinando modelos avancados para Uber Black")
    log.info("=" * 60)
    ...
    log.info("=" * 60)
    log.info("Modelos avancados de Uber Black concluidos")
    log.info("=" * 60)
    return fold_metrics_df, summary_df, comparison_df
